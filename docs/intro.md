---
id: intro
title: AceleraTI — Notas del Programa
sidebar_label: Introducción
sidebar_position: 0
---

# AceleraTI — Notas del Programa

## Presentación del Programa

Acelera TI, una iniciativa de **Bancolombia** y **Pragma**, es un programa diseñado para transformar potencial en experiencia.

Este curso, dictado por **Enyoi**, ha sido desarrollado con un pensum co-creado junto a las necesidades de la industria tecnológica, con foco en **Java Backend** y arquitecturas modernas.

---

## Horario y Duración

| Detalle             | Valor                               |
| ------------------- | ----------------------------------- |
| **Fecha de inicio** | Miércoles 5 de noviembre de 2025    |
| **Días de clase**   | Lunes · Miércoles · Jueves          |
| **Horario**         | 19:00 a 22:00 hrs (Col / Per / Ecu) |
| **Duración total**  | 160 horas sincrónicas               |
| **Frecuencia**      | 9 horas semanales · 17 semanas      |

---

## Currículum

### Parte I — Fundamentos

| #   | Tema                                          |
| --- | --------------------------------------------- |
| 00  | Nivelación: Metodologías Ágiles               |
| 00  | Nivelación: Conceptos Básicos de Programación |
| 01  | Introducción al Proyecto Arka                 |
| 02  | Java                                          |
| 03  | HTTP & REST                                   |
| 04  | Bases de Datos Relacionales                   |
| 05  | Bases de Datos No Relacionales                |
| 06  | Clean Architecture                            |
| 07  | Spring Data JPA                               |
| 08  | Pruebas Unitarias                             |

### Parte II — Arquitectura y Ecosistema

| #   | Tema                                       |
| --- | ------------------------------------------ |
| 09  | Repaso: Clean Architecture + Herramientas  |
| 10  | SOLID & Patrones de Diseño                 |
| 11  | IA & Machine Learning con Java             |
| 12  | Arquitectura Hexagonal                     |
| 13  | Domain Driven Design (DDD)                 |
| 14  | Docker & Docker Compose                    |
| 15  | Patrones de Arquitectura de Microservicios |
| 16  | Repaso Funcional                           |
| 17  | Spring Boot                                |
| 18  | Spring Cloud & Extras                      |
| 19  | AWS                                        |

---

## Proyecto Final: Arka

El proyecto **Arka** es el eje central de la formación. Se trata de una plataforma de e-commerce B2B para una empresa de tecnología que desea migrar sus operaciones físicas al mundo digital.

### Recursos del Proyecto

- [Backlog del proyecto Java Backend Arka](pathname:///assets/PDFs/Backlog%20del%20proyecto%20Java%20Backend%20Arka.pdf)
- [Proyecto Arka — Versión 1](pathname:///assets/PDFs/Proyecto%20Arka%201.pdf)
- [Proyecto Java Backend Reto — Versión 2](pathname:///assets/PDFs/Proyecto%20Java%20Backend%20Reto%20V2.pdf)
- [Guía del Backend Java - Conceptos](pathname:///assets/PDFs/Guía%20del%20Backend%20Java%20-%20Conceptos.pdf)
- [Guía para preparar tu Panel De Expertos](pathname:///assets/PDFs/Guía%20para%20preparar%20tu%20Panel%20De%20Expertos.pdf)

---

## Requisitos de Graduación

1. **Asistencia** — Mínimo 80% durante las 17 semanas
2. **Quices** — 7 quices con 24 horas disponibles cada uno
3. **Proyecto Final** — Sustentación individual ante panel de expertos

### Panel de Expertos

**Fecha:** Semana del 30 de marzo al 3 de abril de 2026

- Sustentación individual guiada por historias de usuario y backlog
- 2 expertos de la industria evalúan apropiación técnica y capacidad de aplicación

---

## Docente

### Andrés Felipe Isaza

- Ingeniero de Sistemas · Especialista en Sistemas de Información
- **Certificaciones:** AWS Certified Solutions Architect · Scrum Master · ITIL V3
- **Rol actual:** Líder Técnico EVC en Bancolombia
- +13 años en transformación digital, arquitectura de soluciones y modernización tecnológica
